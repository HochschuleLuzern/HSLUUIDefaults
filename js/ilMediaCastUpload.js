...
; (function ($, window, document, undefined)
{
    ...
    var ilFile = function (id, file, fileUpload, uploadData)
    {
        ...
        this.getFormData = function ()
        {
            ...
            var visibilityValue = $html.find("input[name=visibility_" + this.id+"]:checked").val();
            ...
            return { title: titleValue, description: descValue, extract: extractValue, keep_structure: keepStructureValue, visibility: visibilityValue };
        };
    ...
    var ilFileUpload = function(id, options)
    {
        ...
        function uploadFailed(e, data)
        {
			...
			if(data.jqXHR!=undefined && (data.jqXHR.responseText.indexOf(file.name)>0 || data.jqXHR.responseText.indexOf(file.uploadData.formData.title)>0)){
				file.setUploadSuccess();
				// all files uploaded?
				if (allUploadsSuccessful())
				{
